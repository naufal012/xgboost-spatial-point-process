...
def make_poisson_metric_lgb(vol_vec):
    def poisson_metric(preds, dtrain):
        ...
        return 'poisson_err', (np.sum(err) + corrections) / 1e6, False
...
def make_logistic_obj_lgb(vol_vec, F_prime_val, hess_min=1e-6):
    def logistic_obj(preds, dtrain):
        ...
        grad[pos_idx] = (-weight[pos_idx] * delta_pos) / (exp_preds_pos * (delta_pos + exp_preds_pos)) + (weight[pos_idx] * delta_pos) / (exp_preds_pos + delta_pos)
        grad[neg_idx] = (weight[neg_idx] * delta_neg) / (exp_preds_neg + delta_neg)
        hess[pos_idx] = (weight[pos_idx] * delta_pos * (2 * exp_preds_pos + delta_pos)) / ((delta_pos * exp_preds_pos + exp_preds_pos**2)**2) + (-weight[pos_idx] * delta_pos) / ((delta_pos + exp_preds_pos)**2)
        hess[neg_idx] = (-weight[neg_idx] * delta_neg) / ((delta_neg + exp_preds_neg)**2)
